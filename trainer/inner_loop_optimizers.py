...
class LSLRGradientDescentLearningRule(nn.Module):
    ...
    def update_params(self, names_weights_dict, names_grads_wrt_params_dict, num_step, tau=0.1, istrain=True):
        """Applies a single gradient descent update to all parameters.
        All parameter updates are performed using in-place operations and so
        nothing is returned.
        Args:
        ...
        if istrain:
            ...
            for key in names_grads_wrt_params_dict.keys():
                if "norm_layer" in key:
                    lr = self.names_learning_rates_dict[key.replace(".", "-")][num_step]
                else:
                    lr = 0
