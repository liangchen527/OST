...
class CelebDFDataset(data.Dataset):
    data_root = './datasets/Celeb-DF/'
    data_list = {'test': './data/CelebDF/test.txt'}
    ...
    def __init__(self, mode='test', res=256, train=False, trainset=None):
        with open(self.data_list[mode], 'r') as fd:
            ...
            img_lines = []
            for line in data:
                label = 1 - int(line.split(' ')[0]) # real:0 fake:1
                name = line.split(' ')[1][:-1]
                step = 2
    ...
    def __getitem__(self, index):
        ...
        label = int(label)
        img = self.load_image(name.split('.')[0], idx)
        ##############
        ##############
        rect = detector(img)
        if len(rect) == 0:
